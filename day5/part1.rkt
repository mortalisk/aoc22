...
(define (move-one stacks from to)
  ...
  (let* ([from (- from 1)]
         ...
         [item (first from-stack)]
         [removed (list-set stacks from (rest from-stack))]
         [added-back (list-set removed to (cons item to-stack))])
  added-back))

(define (move inst stacks)
  (if (= (first inst) 0)
    stacks
    (move (cons (- (first inst) 1) (rest inst)) (move-one stacks (second inst) (third inst)) )))
