...
(define (move-one stacks from to)
  stacks
  (let* ([item (first (list-ref stacks from))]
         [removed (list-set stacks from (rest (list-ref stacks from)))])
    (list-set removed to (cons item (list-ref stacks to)))))

(define (move line stacks)
  (match (regexp-match #rx"move ([0-9]+) from ([0-9]+) to ([0-9]+)" line)
          [#f stacks]
          [(list _ a b c) (let loop ([a (string->number a)][b (- (string->number b) 1)][c (- (string->number c) 1)][stacks stacks])
                            (if (= a 0)
                                stacks
                                (loop (- a 1) b c (move-one stacks b c))))]))
